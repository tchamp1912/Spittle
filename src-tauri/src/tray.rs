...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/spittle.png",
...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, locale: Option<&str>) {
    ...
    let version_label = if cfg!(debug_assertions) {
        format!("Spittle v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Spittle v{}", env!("CARGO_PKG_VERSION"))
    };
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "spittle-1.wav".to_string(),
            timestamp: 0,
            saved: false,
